  ...
  {#if chart && chart.points}
    ...
    <div class="plot-wrap">
      ...
      >
        ...
        <text x="42" y="25" class="panel-label">Altitude (m)</text>
        <polyline points={chart.points} class="snowline-line" />

        <!-- Precipitation panel: mm/3h -->
        {#if chart.hasPrecip}
          <rect x="38" y="154" width="310" height="24" rx="4" class="lower-bg" />
          <text x="34" y="159" text-anchor="end" class="axis precip-axis">{chart.precipMaxLabel}</text>
          <text x="34" y="179" text-anchor="end" class="axis">0</text>
          <text x="42" y="162" class="panel-label precip-label">Precip mm/3h</text>
        ...
        {#if chart.hasSnowDepth}
          <rect x="38" y="184" width="310" height="24" rx="4" class="lower-bg" />
          <text x="34" y="189" text-anchor="end" class="axis depth-axis">{chart.snowDepthMaxLabel}</text>
          <text x="34" y="209" text-anchor="end" class="axis">0</text>
          <text x="42" y="192" class="panel-label depth-label">Modelled snow depth (cm)</text>
          <polyline points={chart.snowDepthPoints} class="snow-depth-line" />
        {/if}

        {#if chart.nowX !== null}
          <line x1={chart.nowX} x2={chart.nowX} y1="14" y2="208" class="now-line" />
        ...
        {#if chart.currentX !== null && chart.currentY !== null}
          <line x1={chart.currentX} x2={chart.currentX} y1="14" y2="208" class="cursor" />
        ...
        {#if chart.crossingX !== null && chart.terrainY !== null && crossing?.crossingTime}
          <line x1={chart.crossingX} x2={chart.crossingX} y1="14" y2="142" class="crossing-line crossing-action" on:click|stopPropagation={() => jumpToCrossing(crossing.crossingTime)} />
        ...
        {#if tooltip}
          <line x1={tooltip.x} x2={tooltip.x} y1="14" y2="208" class="inspect-line" />
          {#if tooltip.snowlineY !== null}<circle cx={tooltip.x} cy={tooltip.snowlineY} r="3.8" class="inspect-dot" />{/if}
        {/if}

        <text x="38" y="228" text-anchor="start" class="axis">{chart.startLabel}</text>
        <text x="193" y="228" text-anchor="middle" class="axis">+72 h</text>
        <text x="348" y="228" text-anchor="end" class="axis">+144 h</text>
      </svg>
...
<script lang="ts">
  ...
  function clampPosition(x: number, y: number) { const rect = chartShell?.getBoundingClientRect(); const width = rect?.width ?? 390; const height = rect?.height ?? 350; return { x: Math.max(6, Math.min(window.innerWidth - width - 6, x)), y: Math.max(6, Math.min(window.innerHeight - height - 6, y)) }; }
  ...
  async function downloadPng() {
    ...
    try {
      ...
      const canvas = document.createElement('canvas'); canvas.width = 1080; canvas.height = 880;
      ...
      ctx.font = '19px Arial, sans-serif'; ctx.fillText('Snowline   ·   Terrain   ·   Precipitation   ·   Modelled snow depth', 42, 122);
      ctx.drawImage(image, 0, 138, 1080, 708);
      ctx.fillStyle = '#87929e'; ctx.font = '16px Arial, sans-serif'; ctx.fillText('Snowline is a thermal boundary; local snowfall and accumulation may differ.', 42, 866);
  ...
  function handlePlotPointer(event: PointerEvent) {
    ...
    const cssX = Math.max(74, Math.min(rect.width - 74, (x / 360) * rect.width)); const cssY = Math.max(8, Math.min(rect.height - 104, (snowlineY !== null ? (snowlineY / 236) * rect.height - 78 : 24)));
  ...
  function buildChart(p: any, terrain: number | null, target: number, crossingTime: number | null, realNowTime: number): ChartData | null {
    ...
    // One authoritative selected index drives the marker, footer, subtitle and Windy timeline values.
    ...
    const spacing = (right - left) / Math.max(1, p.times.length - 1), barWidth = Math.max(1.2, Math.min(5, spacing * 0.72)); const precipBars: PrecipBar[] = precipValues.map((mm: number | null, index: number) => { const value = mm ?? 0, height = precipMax > 0 ? Math.min(24, (value / precipMax) * 24) : 0; return { x: x(p.times[index]) - barWidth / 2, y: 178 - height, width: barWidth, height, mm: value }; }).filter(bar => bar.height > 0.15);
    ...
    const snowDepthPoints = snowDepthValues.map((cm: number | null, index: number) => cm === null ? null : `${x(p.times[index]).toFixed(1)},${(208 - Math.min(24, (cm / snowDepthMax) * 24)).toFixed(1)}`).filter((v: string | null): v is string => v !== null).join(' ');
  ...
  onMount(() => { const width = Math.min(390, window.innerWidth - 24); position = { x: Math.max(6, (window.innerWidth - width) / 2), y: window.innerWidth <= 520 ? 52 : 64 }; realNow = Date.now(); realNowTimer = setInterval(() => { realNow = Date.now(); }, 30_000); try { const current = store.get('timestamp'); if (typeof current === 'number' && Number.isFinite(current)) timestamp = current; timestampListener = store.on('timestamp', (value: any) => { const next = Number(value); if (Number.isFinite(next)) timestamp = next; }); } catch {} });
...
<style lang="less">
  ...
  .snowline-chart-legend { display: flex; flex-wrap: wrap; gap: 7px; margin: 6px 2px 1px; color: rgba(255,255,255,0.68); font-size: 7.8px; }
  ...
  .axis { fill: rgba(255,255,255,0.52); font-size: 8px; font-family: sans-serif; } .panel-label { fill: rgba(255,255,255,0.60); font-size: 6.8px; font-family: sans-serif; font-weight: 700; } .precip-axis, .precip-label { fill: rgba(112,215,255,0.90); } .depth-axis, .depth-label { fill: #8fe388; }
  ...
  .chart-foot { display: grid; grid-template-columns: repeat(4, 1fr); gap: 4px; margin-top: 1px; } .chart-foot span { padding: 5px 3px; border-radius: 6px; background: rgba(255,255,255,0.045); color: rgba(255,255,255,0.60); text-align: center; font-size: 7.2px; min-width: 0; } .chart-foot b { display: block; color: white; font-size: 8.2px; white-space: nowrap; } .chart-foot b.positive { color: #70d7ff; } .chart-foot b.negative { color: #ffb15b; } .chart-foot b.wet { color: #70d7ff; } .chart-foot b.depth-value { color: #8fe388; }
