...
<script lang="ts">
  ...
  function updatePersistentClickLabel() {
    ...
    const rounded = Math.round(snowline / 10) * 10, tendency = tendencyText(clickedPoint, idx), precip = precipMmAt(clickedPoint.forecast, idx);
    ...
    showClickLabel(lat, lon, `${rounded} m`, `${tendency}${precipText}`, colorForLevel(snowline));
  }
