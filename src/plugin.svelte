...
<script lang="ts">
  ...
  function openWindyForecast(lat: number, lon: number) { try { bcast.emit('rqstOpen' as any, 'detail' as any, { lat, lon, model: MODEL, step: 1, days: FORECAST_DAYS } as any); } catch { try { (singleclick as any).fire('click', { lat, lon }); } catch {} } }
  ...
  function showClickLabel(lat: number, lon: number, mainText: string, detailText = '', snowlineColor = '#ffffff', status: ProbeStatus = 'neutral', outlookText = '') {
    ...
    marker.on('click', (event: any) => {
      ...
      const forecast = target?.closest?.('.snowline-label-forecast'), graph = target?.closest?.('.snowline-label-chart'), share = target?.closest?.('.snowline-label-share') as HTMLButtonElement | null, close = target?.closest?.('.snowline-label-close');
