...
<script lang="ts">
  ...
  const COLOUR_STOPS: ColourStop[] = [
    { value: 150, color: '#c51ac7' }, { value: 300, color: '#8b079e' },
    { value: 450, color: '#50007f' }, { value: 600, color: '#231073' },
    { value: 750, color: '#003e91' }, { value: 1000, color: '#1688d4' },
    { value: 1300, color: '#72bdf3' }, { value: 1600, color: '#b9e7c7' },
    { value: 1900, color: '#c8ef4a' }, { value: 2200, color: '#f4eb00' },
    { value: 2500, color: '#ffc21a' }, { value: 2800, color: '#ff850d' },
  ...
  function getStoreTimestamp(): number {
    try { const t = store.get('timestamp'); if (typeof t === 'number' && Number.isFinite(t)) return t; } catch {}
    return Date.now();
  }
  function parseTime(value: unknown): number | null {
    if (typeof value === 'number' && Number.isFinite(value)) { if (value > 1e12) return value; if (value > 1e9) return value * 1000; }
  ...
  function buildForecastTimes(data: Record<string, unknown>, header: Record<string, unknown>): number[] {
    const hours = data['hours']; if (hours == null) return [];
    const n = Number((hours as any).length); if (!Number.isFinite(n) || n <= 0) return [];
    const raw: number[] = [];
    for (let i = 0; i < n; i++) { const v = valueAt(hours, i); if (v !== null) raw.push(v); }
    if (!raw.length) return [];
    let times: number[];
    if (raw[0] > 1e12) times = raw;
    else if (raw[0] > 1e9) times = raw.map(v => v * 1000);
    else { const ref = parseTime(header.refTime); if (ref === null) return []; times = raw.map(h => ref + h * 3600_000); }
  ...
  function nearestIndex(times: number[], target: number): number {
    ...
    times.forEach((t, i) => { const d = Math.abs(t - target); if (d < best) { best = d; bestIndex = i; } });
    return bestIndex;
  }
  ...
  function precipitationKeys(data: Record<string, unknown>): string[] {
    const exact = [
      'precip-surface', 'past3hprecip-surface', 'rain-surface', 'snow-surface',
      'snowprecip-surface', 'convectivePrecip-surface', 'convectiveprecip-surface', 'tp-surface'
    ].filter(key => key in data);
    if (exact.length) return exact;
    return Object.keys(data).filter(key => /(^|[-_])(precip|rain|snow|tp)([-_]|$)/i.test(key));
  }
  function precipitationAt(data: Record<string, unknown>, idx: number): number | null {
    const keys = precipitationKeys(data);
    if (!keys.length) return null;
    let sum = 0, found = false;
    for (const key of keys) {
      const v = valueAt(data[key], idx);
      if (v !== null && Number.isFinite(v)) { sum += Math.max(0, v); found = true; }
    }
    return found ? sum : null;
  }
  function hasPrecipitation(data: Record<string, unknown>, idx: number): boolean | null {
    const p = precipitationAt(data, idx);
    return p === null ? null : p > PRECIP_EPSILON;
  }

  async function loadMapElevation(lat: number, lon: number): Promise<number | null> {
    try {
      const p = await getElevation(lat, lon) as any;
      for (const candidate of [p?.data, p?.data?.data, p?.value]) { const elevation = scalarNumber(candidate); if (elevation !== null) return elevation; }
    } catch (e) { console.warn('Snowline map elevation failed', lat, lon, e); }
    return null;
  }
  ...
  function invalidateForNewRun(runTime: number | null) {
    ...
    activeRunTime = runTime; profileCache.clear(); cache = []; clickedPoint = null; clickGeneration += 1;
  }
  function rememberProfile(point: CachedPoint) {
    const key = profileKey(point.lat, point.lon); profileCache.delete(key); profileCache.set(key, point);
    while (profileCache.size > PROFILE_CACHE_MAX) { const oldest = profileCache.keys().next().value; if (oldest === undefined) break; profileCache.delete(oldest); }
  }
  function cachedProfile(lat: number, lon: number): CachedPoint | null {
    const key = profileKey(lat, lon), point = profileCache.get(key); if (!point) return null;
    if (activeRunTime !== null && point.runTime !== null && Math.abs(point.runTime - activeRunTime) >= 60_000) { profileCache.delete(key); return null; }
    profileCache.delete(key); profileCache.set(key, point); return point;
  }
  async function loadPoint(lat: number, lon: number): Promise<CachedPoint | null> {
    const existing = cachedProfile(lat, lon); if (existing) return existing;
    try {
      const response = await getMeteogramForecastData(MODEL, { lat, lon, step: 1, days: FORECAST_DAYS });
      const { forecast, header } = extractPayload(response); if (!Object.keys(forecast).length) return null;
      const runTime = parseTime(header.refTime); invalidateForNewRun(runTime);
      const point: CachedPoint = { lat, lon, forecast, header, times: buildForecastTimes(forecast, header), runTime };
      rememberProfile(point); return point;
    } catch (e) { console.warn('Snowline point failed', lat, lon, e); return null; }
  }
  ...
  function gridShapeForZoom(): { rows: number; cols: number } {
    const zoom = Number(map.getZoom?.() ?? 6), mapWidth = Number(map.getSize?.().x ?? 800);
    if (zoom <= 4) return { rows: 9, cols: 13 }; if (zoom <= 7) return { rows: 11, cols: 17 };
    if (mapWidth < 520) return { rows: 13, cols: 19 }; return { rows: 15, cols: 21 };
  }
  function buildViewportPoints(): { points: ViewportPoint[]; rows: number; cols: number } {
    const { rows, cols } = gridShapeForZoom(), b = map.getBounds();
    const south = Math.max(-75, b.getSouth()), north = Math.min(75, b.getNorth()), west = b.getWest(), east = b.getEast();
    const latStep = (north - south) / (rows - 1), lonStep = (east - west) / (cols - 1), points: ViewportPoint[] = [];
  ...
  async function refreshViewport() {
    ...
    try {
      ...
      cache = nextCache; renderFromCache();
    } finally { if (myGeneration === generation) viewportLoading = false; }
  }
  ...
  function statusColor(status: ProbeStatus): string {
    if (status === 'above') return '#46d9ff'; if (status === 'below') return '#ff9d3d'; if (status === 'near') return '#ffe45c'; if (status === 'dry') return '#b7bdc7'; return '#ffffff';
  }
  function showClickLabel(lat: number, lon: number, mainText: string, detailText = '', snowlineColor = '#ffffff', status: ProbeStatus = 'neutral') {
    ...
    L.marker([lat, lon], { interactive: false, zIndexOffset: 2000, icon: L.divIcon({ className: `snowline-click-label snowline-probe-${status}`, html: `<span style="--snowline-color:${snowlineColor};--probe-accent:${accent}"><b>${mainText}</b>${detail}</span>`, iconSize: [196, 58], iconAnchor: [98, 66] }) }).addTo(clickLayer);
  }

  function snowlineAt(point: CachedPoint, idx: number): number | null {
    const wbz = wetBulbZeroHeight(buildProfile(point.forecast, idx));
    return wbz.snowLevelM !== null && Number.isFinite(wbz.snowLevelM) ? wbz.snowLevelM : null;
  }
  function tendencyText(point: CachedPoint, idx: number): string {
    const now = snowlineAt(point, idx); if (now === null) return '';
    const target = point.times[idx] + TENDENCY_HOURS * 3600_000;
    if (target > point.times[point.times.length - 1] + 30 * 60_000) return '';
    const futureIdx = nearestIndex(point.times, target);
    if (futureIdx === idx) return '';
    const future = snowlineAt(point, futureIdx); if (future === null) return '';
    const delta = Math.round((future - now) / 10) * 10;
    if (Math.abs(delta) < 20) return `Snowline steady / ${TENDENCY_HOURS} h`;
    return `Snowline ${delta > 0 ? '↑' : '↓'} ${Math.abs(delta)} m / ${TENDENCY_HOURS} h`;
  }
  ...
  function updatePersistentClickLabel() {
    ...
    const precip = hasPrecipitation(clickedPoint.forecast, idx);
    ...
    if (clickedMapElevationM !== null && Number.isFinite(clickedMapElevationM)) {
      ...
      if (difference > NEAR_SNOWLINE_METRES) { status = 'above'; headline = '↑ ABOVE SNOWLINE'; }
      else if (difference < -NEAR_SNOWLINE_METRES) { status = 'below'; headline = '↓ BELOW SNOWLINE'; }
      else { status = 'near'; headline = '≈ NEAR SNOWLINE'; }
      const detail = `Here ${terrainRounded} m · Snowline ${rounded} m${tendency ? ` · ${tendency}` : ''}`;
      showClickLabel(lat, lon, headline, detail, colorForLevel(snowline), status); return;
    }
  ...
  async function probeLocation(lat: number, lon: number) {
    ...
    clickedPoint = null; clickedMapElevationM = null; clickedLatLon = [lat, lon]; probeLoading = true;
  ...
  function midpointAlongLine(line: ContourPolyline): [number, number] | null {
    if (line.length < 2) return null; const lengths: number[] = []; let total = 0;
    for (let i = 1; i < line.length; i++) { const a = line[i - 1], b = line[i], meanLat = (a[0] + b[0]) * 0.5 * Math.PI / 180, d = Math.hypot((b[1] - a[1]) * Math.cos(meanLat), b[0] - a[0]); lengths.push(d); total += d; }
    if (total <= 0) return line[Math.floor(line.length / 2)];
    const target = total / 2; let travelled = 0;
    for (let i = 0; i < lengths.length; i++) { const d = lengths[i]; if (travelled + d >= target) { const f = d > 0 ? (target - travelled) / d : 0, a = line[i], b = line[i + 1]; return [a[0] + f * (b[0] - a[0]), a[1] + f * (b[1] - a[1])]; } travelled += d; }
    return line[line.length - 1];
  }
  function drawDeclutteredLabels(candidates: LabelCandidate[], layer: any) {
    if (!candidates.length) return;
    const mapWidth = Number(map.getSize?.().x ?? 800), maxLabels = mapWidth < 520 ? 5 : mapWidth < 900 ? 7 : 10, minDistance = mapWidth < 520 ? 82 : LABEL_MIN_DISTANCE_PX;
    const ordered = [...candidates].sort((a, b) => a.isMajor !== b.isMajor ? (a.isMajor ? -1 : 1) : b.length - a.length), occupied: { x: number; y: number }[] = []; let placed = 0;
    for (const candidate of ordered) {
      if (placed >= maxLabels) break; const px = map.latLngToContainerPoint(candidate.point); if (occupied.some(other => Math.hypot(px.x - other.x, px.y - other.y) < minDistance)) continue;
      ...
      occupied.push({ x: px.x, y: px.y }); placed += 1;
    }
  }
  function renderFromCache() {
    ...
    for (let r = 0; r < cache.length; r++) {
      ...
      for (let c = 0; c < cache[r].length; c++) {
        const cp = cache[r][c];
        if (!cp || !cp.times.length) { row.push({ lat: 0, lon: 0, value: null }); continue; }
        const idx = nearestIndex(cp.times, target), precip = hasPrecipitation(cp.forecast, idx);
        const snowline = precip === true ? snowlineAt(cp, idx) : null;
        row.push({ lat: cp.lat, lon: cp.lon, value: snowline });
      }
      field.push(row);
    }
    ...
    const nextLayer = L.layerGroup(), min = Math.floor(Math.min(...values) / interval) * interval, max = Math.ceil(Math.max(...values) / interval) * interval, labelCandidates: LabelCandidate[] = [];
    for (let level = min; level <= max; level += interval) {
      ...
      for (const line of lines) {
        ...
        if (is500 || is1000) L.polyline(line, { color: '#11151b', weight: is1000 ? 4.3 : 3.0, opacity: is1000 ? 0.58 : 0.42, interactive: false, lineCap: 'round', lineJoin: 'round', smoothFactor: 0.8 }).addTo(nextLayer);
        L.polyline(line, { color: contourColor, weight: is1000 ? 2.9 : is500 ? 1.9 : 0.9, opacity: is1000 ? 1.0 : is500 ? 0.96 : 0.68, interactive: false, lineCap: 'round', lineJoin: 'round', smoothFactor: is500 ? 0.7 : 0.9 }).addTo(nextLayer);
      }
      const shouldLabel = interval === 100 ? is500 : is1000;
      if (shouldLabel && lines.length) {
        const longest = [...lines].sort((a, b) => lineLength(b) - lineLength(a))[0], length = lineLength(longest), labelPoint = midpointAlongLine(longest);
        if (labelPoint && length > 0.08) labelCandidates.push({ point: labelPoint, level, color: contourColor, length, isMajor: is1000 });
      }
    }
    drawDeclutteredLabels(labelCandidates, nextLayer); nextLayer.addTo(map);
    const oldLayer = contourLayer; contourLayer = nextLayer; if (oldLayer) try { map.removeLayer(oldLayer); } catch {}
  }
  ...
  function setDisplayMode(mode: DisplayMode) {
    ...
    displayMode = mode; generation += 1; viewportLoading = false;
    if (!contoursEnabled()) clearContours(); else refreshViewport();
    if (!labelsEnabled()) clearPointState();
    else if (followPicker) { try { schedulePickerProbe(store.get('pickerLocation'), true); } catch {} }
  }
  function toggleFollowPicker() {
    if (!enabled || !labelsEnabled()) return;
    if (followPicker) { try { schedulePickerProbe(store.get('pickerLocation'), true); } catch {} }
  }
  function toggleEnabled() {
    if (enabled) {
      if (contoursEnabled()) refreshViewport();
      if (labelsEnabled() && followPicker) { try { schedulePickerProbe(store.get('pickerLocation'), true); } catch {} }
    } else {
      generation += 1; viewportLoading = false; if (pickerTimer) { clearTimeout(pickerTimer); pickerTimer = null; } clearContours(); clearPointState();
    }
  }
  ...
  onMount(() => {
    ...
    try {
      ...
      if (labelsEnabled() && followPicker) schedulePickerProbe(store.get('pickerLocation'), true);
    } catch (e) { console.warn('Snowline picker-location listener unavailable', e); }
    if (contoursEnabled()) refreshViewport();
  });
  onDestroy(() => {
    generation += 1; clickGeneration += 1;
    if (moveTimer) clearTimeout(moveTimer); if (pickerTimer) clearTimeout(pickerTimer);
...
<style lang="less">
  ...
  :global(.snowline-click-label small) { font-size: 9.1px; line-height: 1.15; font-weight: 700; opacity: 0.9; }
