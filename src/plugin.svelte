...
<script lang="ts">
  ...
  function tendencyText(point: CachedPoint, idx: number): string { const now = snowlineAt(point, idx); if (now === null) return ''; const target = point.times[idx] + TENDENCY_HOURS * 3600_000; if (target > point.times[point.times.length - 1] + 30 * 60_000) return ''; const futureIdx = nearestIndex(point.times, target); if (futureIdx === idx) return ''; const future = snowlineAt(point, futureIdx); if (future === null) return ''; const delta = Math.round((future - now) / 10) * 10; if (Math.abs(delta) < 20) return `Snowline steady / ${TENDENCY_HOURS} h`; return `Snowline ${delta > 0 ? '↑' : '↓'} ${Math.abs(delta)} m / ${TENDENCY_HOURS} h`; }
