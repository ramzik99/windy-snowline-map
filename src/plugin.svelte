...
<script lang="ts">
  ...
  function updatePersistentClickLabel() {
    ...
    const idx = nearestIndex(clickedPoint.times, target), wbz = wetBulbZeroHeight(buildProfile(clickedPoint.forecast, idx));
    if (wbz.snowLevelM === null || !Number.isFinite(wbz.snowLevelM)) { showClickLabel(lat, lon, 'No snowline'); return; }
    const snowline = wbz.snowLevelM, rounded = Math.round(snowline / 10) * 10;
    if (clickedMapElevationM !== null && Number.isFinite(clickedMapElevationM)) {
      ...
      showClickLabel(lat, lon, headline, `Here ${terrainRounded} m · Snowline ${rounded} m`, colorForLevel(snowline), status); return;
    }
  ...
  function drawDeclutteredLabels(candidates: LabelCandidate[], layer: any) {
    if (!candidates.length) return; const mapWidth = Number(map.getSize?.().x ?? 800), maxLabels = mapWidth < 520 ? 5 : mapWidth < 900 ? 7 : 10, minDistance = mapWidth < 520 ? 82 : LABEL_MIN_DISTANCE_PX;
    const ordered = [...candidates].sort((a, b) => a.is1000 !== b.is1000 ? (a.is1000 ? -1 : 1) : b.length - a.length), occupied: { x: number; y: number }[] = []; let placed = 0;
  ...
  function renderFromCache() {
    ...
    for (let r = 0; r < cache.length; r++) {
      const row: GridPoint[] = [];
      for (let c = 0; c < cache[r].length; c++) { const cp = cache[r][c]; if (!cp || !cp.times.length) { row.push({ lat: 0, lon: 0, value: null }); continue; } const idx = nearestIndex(cp.times, target), wbz = wetBulbZeroHeight(buildProfile(cp.forecast, idx)); row.push({ lat: cp.lat, lon: cp.lon, value: wbz.snowLevelM }); }
      field.push(row);
    }
    const values = field.flat().map(p => p.value).filter((v): v is number => typeof v === 'number' && Number.isFinite(v)); if (!values.length) return;
    const nextLayer = L.layerGroup(), min = Math.floor(Math.min(...values) / CONTOUR_INTERVAL) * CONTOUR_INTERVAL, max = Math.ceil(Math.max(...values) / CONTOUR_INTERVAL) * CONTOUR_INTERVAL, labelCandidates: LabelCandidate[] = [];
    for (let level = min; level <= max; level += CONTOUR_INTERVAL) {
      ...
      for (const line of lines) {
        ...
        if (is500) L.polyline(line, { color: '#11151b', weight: is1000 ? 4.3 : 3.0, opacity: is1000 ? 0.58 : 0.42, interactive: false, lineCap: 'round', lineJoin: 'round', smoothFactor: 0.8 }).addTo(nextLayer);
        L.polyline(line, { color: contourColor, weight: is1000 ? 2.9 : is500 ? 1.9 : 0.82, opacity: is1000 ? 1.0 : is500 ? 0.96 : 0.62, interactive: false, lineCap: 'round', lineJoin: 'round', smoothFactor: is500 ? 0.7 : 0.9 }).addTo(nextLayer);
      }
      if (is500 && lines.length) { const longest = [...lines].sort((a, b) => lineLength(b) - lineLength(a))[0], length = lineLength(longest), labelPoint = midpointAlongLine(longest); if (labelPoint && length > 0.08) labelCandidates.push({ point: labelPoint, level, color: contourColor, length, is1000 }); }
    }
...
<style lang="less">
  ...
  :global(.snowline-click-label small) { font-size: 9.5px; line-height: 1.1; font-weight: 700; opacity: 0.9; }
