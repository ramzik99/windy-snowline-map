...
replacements = [
(
"  type LabelGridArgs = { terrain:number; snowline:number; difference:number; precip:number|null; hasPrecip:boolean; eventLine:string };",
...
"        <div><b>Point card:</b> gives the selected terrain elevation, current snowline relationship and precipitation, plus one concise line for the next terrain-relevant wintry event through +144 h.</div>"
),
(
...
(
"iconSize:[224,166],iconAnchor:[112,174]",
"iconSize:[224,176],iconAnchor:[112,184]"
),
(
":global(.snowline-click-label>span){position:relative;display:flex;flex-direction:column;gap:5px;width:224px;min-height:132px;box-sizing:border-box;padding:37px 8px 8px;",
...
if old not in text:
    raise SystemExit('Expected shareCurrentPoint preamble not found')
text = text.replace(old, new, 1)
...
if old not in text:
    raise SystemExit('Expected legacy 24 h share fields not found')
text = text.replace(old, new, 1)
